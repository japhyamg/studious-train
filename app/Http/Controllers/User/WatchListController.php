...
class WatchListController extends Controller implements HasMiddleware
{
    ...
    public function upload(Request $request)
    {
        ...
        try {
            $stats = (new WatchListImportService())->importInternal($request->file('file'));

            activity()->log("Internal watchlist import: {$stats['created']} created, {$stats['skipped']} skipped, {$stats['errors']} errors.");

            return redirect(route('watch-list.internal.all'))->with(
                'success',
                "Import complete: {$stats['created']} added, {$stats['skipped']} skipped, {$stats['errors']} failed."
            );
        } catch (\Throwable $e) {
            return redirect(route('watch-list.internal.all'))->with('error', 'Import failed: ' . $e->getMessage());
        }
    }
}
